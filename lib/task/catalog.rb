module Task
  ...
  def self.dump_catalog(context, resources_filter = [])
  ...
  # @return [String]
  def self.parameter_value_format(value)
    case value
      when TrueClass then
        'true'
      ...
      when Numeric, Symbol then
        parameter_value_format value.to_s
      when String then
        begin
  ...
  # @return [String]
  def self.dump_resource(resource)
  ...
  # @param data [String]
  # @return [String]
  def self.preprocess_catalog_data(data)
    clear_data = []
    data.to_s.split("\n").each do |line|
      line = line.rstrip
      next if line == ''
  ...
  def self.resources_are_same?(res1, res2)
  ...
  # @return [String]
  def self.catalog_manifest_base_dir
  ...
  # @param context [Object] the context from the rspec test
  # @return [String]
  def self.catalog_dump_file_path(context)
  ...
  def self.save_catalog_to_file(context, resources_filter)
  ...
  # @param context [Object] the context from the rspec test
  def self.read_catalog_from_file(context)
    file_path = catalog_dump_file_path(context)
