...
module Task
  ...
  def self.rspec_command
    command = []
    if parallel_enabled?
      command += %w(rspec -f json)
    else
      command += %w(rspec -f doc -c --tty -b)
    end
  ...
  def self.inside_noop_root_path
  ...
  # and then return back
  def self.inside_deployment_path
    current_directory = Dir.pwd
  ...
  def self.parallel_enabled?
    options.parallel_run and options.parallel_run > 0
  end

  def self.proc_title=(value)
  ...
  def self.full_spec_path(spec)
    File.join spec_hosts_path, spec
  end

  def self.run(yaml, *args)
    env = {ASTUTE_YAML_VAR => yaml}
    p env
    if parallel_enabled?
      *out = Open3.capture2e(env, *args.flatten)
      {
          :success => out.last.exitstatus == 0,
          :report => out.first,
      }
    else
  ...
  def self.rspec(yaml, *spec)
    ...
    result = run yaml, command
    puts "Finish: #{command.flatten.join ' '} (#{yaml}) [success: #{result[:success]}]"
    result
  end

  def self.run_rspec_individually(yaml)
    parallel_results = Parallel.map(
      ...
      result = rspec yaml, full_spec_path(spec)
      result[:spec] = spec
      result
    end
    report = {
  ...
  def self.run_rspec_with_pattern(yaml)
  ...
  def self.astute_yaml_iteration
    # prepare_bundle if options.bundle
    self.proc_title = 'Noop tests Master'
    ...
    parallel_results = Parallel.map(
      ...
      self.proc_title = "Noop tests for YAML: '#{astute_yaml}'"
      globals astute_yaml
      result = yield
      [astute_yaml, result]
    end
