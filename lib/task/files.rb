module Task
  ...
  def self.catalog_file_resources_report(context)
    files = {}
  ...
  def self.save_file_resources_list_to_file(context)
  ...
  def self.binary_files_report_template(binding)
    template = <<-'eos'
    <% if binary_files.any? -%>
  ...
  def self.check_for_binary_files_installation(context)
